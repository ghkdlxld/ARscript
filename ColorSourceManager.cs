...

{
    ...
    //get은 public, set은 private 변경불가 -> 받아온 값을 저장
    ...
    void Start()
    {
        ...
        if (_Sensor != null) //_Sensor가 채워져 있으면
        {
            ...
            _Data = new byte[frameDesc.BytesPerPixel * frameDesc.LengthInPixels]; //1픽셀에 해당하는 바이트 수*픽셀의 길이
    ...
    void Update () 
    {
        ...
        {
            var frame = _Reader.AcquireLatestFrame(); //변수 frame = _Reader의 가장 최신 프레임

            if (frame != null) //frame 채워져있으면
            {
                frame.CopyConvertedFrameDataToArray(_Data, ColorImageFormat.Rgba); //색상 정보를 보유하는 배열을 복사해줌
                 _Texture.LoadRawTextureData(_Data); //수치화된 TextureData 로드
                _Texture.Apply(); //Texture 인자를 하나로 묶어 배열로 만들어 넣는 것 //로드한거 적용
